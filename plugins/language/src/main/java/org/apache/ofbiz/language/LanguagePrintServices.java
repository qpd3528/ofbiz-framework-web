...
public class LanguagePrintServices {
    private static final String MODULE = LanguagePrintServices.class.getName();
    private static final String RESOURCE = "LanguageUiLabels";

    public static Map<String, Object> printReportPdf(DispatchContext dctx, Map<String, ? extends Object> context) {
        Locale locale = (Locale) context.get("locale");
        String screenLocation = "component://language/widget/language/LanguageReportScreens.xml";
        String reportScreenName = "LanguageReport";
        ...
        try {
            ...
            String errMsg = UtilProperties.getMessage(RESOURCE, "LanguageGeneralErrorRenderingScreen", UtilMisc.toMap("errorString", e.toString()),
                    locale);
            Debug.logError(e, errMsg, MODULE);
            return ServiceUtil.returnError(errMsg);
        } catch (IOException e) {
            String errMsg = UtilProperties.getMessage(RESOURCE, "LanguageIOErrorRenderingScreen", UtilMisc.toMap("errorString", e.toString()), locale);
            Debug.logError(e, errMsg, MODULE);
            return ServiceUtil.returnError(errMsg);
        }
        ...
        // put the languageId in the screen context, is a parameter coming into the service
        //reportScreenRenderer.getContext().put("languageId", context.get("languageId"));
        ...
        try {
            ...
            String errMsg = UtilProperties.getMessage(RESOURCE, "LanguageSAXErrorRenderingScreen", UtilMisc.toMap("errorString", e.toString()),
                    locale);
            Debug.logError(e, errMsg, MODULE);
            return ServiceUtil.returnError(errMsg);
        } catch (ParserConfigurationException e) {
            String errMsg = UtilProperties.getMessage(RESOURCE, "LanguageParserConfigurationErrorRenderingScreen", UtilMisc.toMap("errorString",
                    e.toString()), locale);
            Debug.logError(e, errMsg, MODULE);
            return ServiceUtil.returnError(errMsg);
        }
        ...
        */
        ...
        if (services.length > 0) {
            ...
            try {
                ...
                String errMsg = UtilProperties.getMessage(RESOURCE, "LanguageUnableToPrintPDFFromXSL-FO", UtilMisc.toMap("errorString",
                        pe.toString()), locale);
